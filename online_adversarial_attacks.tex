...
\cut{
In such a scenario attacking every possible data point (e.g., video frame) is risky as the attacker may be detected by a defense mechanism and every additional attacked instance further increases this risk. Generally speaking, as conventional adversarial attacks operate by restricting the perturbation to a fraction of the maximum possible change (e.g., $\ell_{\infty}$ attacks) online attacks analogously restrict the time window to a fraction of possible instances to attack. Thus we argue that the spirit of adversarial attacks--i.e. the difficulty of being detected--in this online setting is well captured by an online attack budget. Also, we note similar constraints were considered in the RL setting (i.e. attacking a subset of states) by \cite{lin2017tactics, sun2020stealthy}. However, it is important to note this is a factor that the adversary can control in practice. In the autonomous control system example described above the adversary can choose to be active for a short interval--e.g., when the car is a certain geospatial location-- and thus set the value for $N$.}
